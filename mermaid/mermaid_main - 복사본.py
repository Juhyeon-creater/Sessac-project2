...
START_BENDING_THRESH = 15   # 운동 시작으로 간주하는 각도
CENTER_ANGLE_THRESH = 10.0  # 정자세(복귀)로 간주하는 각도
SHOULDER_LEVEL_THRESH = 0.1 
PELVIS_SENSOR_THRESH = 5.0  

# [운동 루틴 설정]
TARGET_SETS = 3
HOLD_DURATION = 5.0 # 한쪽 방향 유지 시간
...
# ===============================
def get_font(size):
    try: return ImageFont.truetype("malgun.ttf", size) # 윈도우용 폰트
...
class MermaidRoutine:
    def __init__(self):
        self.state = "IDLE" # IDLE, PREP, LEFT_WAIT, LEFT_HOLD, RECOVER_1, RIGHT_WAIT, RIGHT_HOLD, RECOVER_2, FINISHED
        self.current_set = 1
    ...
    def update(self, angle, direction, is_stable, now):
        ...
        if self.state == "PREP":
            if now - self.state_start_time > 3.0:
                self.state = "LEFT_WAIT"
                return "1세트 시작", (0, 255, 255), "왼쪽으로 기울이세요", "왼쪽으로 몸을 기울여 주세요.", None
        ...
        if self.state == "RECOVER_1":
            if angle < CENTER_ANGLE_THRESH: # 정자세 복귀 확인
        ...
        if self.state == "RECOVER_2":
            if angle < CENTER_ANGLE_THRESH:
                if self.current_set >= self.target_sets:
                    ...
                    self.state = "LEFT_WAIT"
                    time.sleep(5.0)
                    return f"{self.current_set}세트 시작", (0, 255, 255), "다음 세트 왼쪽 준비", f"{self.current_set}세트 시작. 왼쪽으로 기울여 주세요.", None
            return "복귀 중", (200, 200, 200), "허리를 세우세요", None, None
...
def run_mermaid_coach():
    ...
    try:
        while True:
            ...
            draw.text((x_pos, y_pos), text, font=font_guide, fill=dark_brown, stroke_fill=dark_brown)
